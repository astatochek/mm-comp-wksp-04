...
def NewtonCondition(segment):
    ...
    for i in range(n):
        cur = a + (i + 1) * h
        if (df(a) * df(cur) > 0 and df(a) * df(b) > 0) or (df(a) * df(cur) < 0 and df(a) * df(b) < 0):
            continue
        elif (ddf(a) * ddf(cur) > 0 and ddf(a) * ddf(b) > 0) or (ddf(a) * ddf(cur) < 0 and ddf(a) * ddf(b) < 0):
            continue
        else:
            return False
    return True
...
def PrepareOutput(name, beg, cnt, res, nev):
    print(f"{name}:")
